var zoom = (function ($) {
  var selectors = {
    ...
    images = [
      ...
      ['telegram.png', [-0.02, 0, 2500, false, true], 0.3, [2200, 2221]],
  ...
  function setDimensions() {
    ...
    if (utils.isMobile()) {
      dimensions.points.width = 0;
      dimensions.points.outline = 0;
      dimensions.points.label.width = 0;
      dimensions.points.label.height = 0;
    } else {
      dimensions.points.width = 0;
      dimensions.points.outline = 0;
      dimensions.points.label.width = 0;
        ...
        switch (layer.type) {
          ...
          case 'pattern':
            var pattern = context.createPattern(layer, 'repeat'),
              parallax =
                ((dimensions.height * -(-offsetZ - layer.fade[0])) /
                  (layer.fade[1] - layer.fade[0])) *
                  layer.fade[3] -
                tilt * 0.1;
            parallax =
              dimensions.height * -(tilt / getTilt(-dimensions.totalHeight)) * layer.fade[3];
            context.fillStyle = pattern;
            context.save();
            context.translate(0, -parallax);
            context.fillRect(
              layer.coords[0],
              layer.coords[1] - dimensions.height,
              layer.width,
              layer.height * 2,
            );
            context.restore();
  ...
  function drawPoint(context, point) {
    ...
    var textX =
        (point.ltr
          ? x + side + side * dimensions.points.xAngle
          : x -
            side * dimensions.points.xAngle -
            dimensions.points.label.width * scale * dimensions.pixelRatio) +
        10 * scale * dimensions.pixelRatio * (point.ltr ? 1 : -1),
    ...
    if (active) {
      if (utils.isMobile()) {
        ...
        point.clickTarget[1] =
          textX >= point.coords[2]
            ? textX + dimensions.points.label.width * scale * dimensions.pixelRatio
            : point.clickTarget[1];
      } else {
        point.clickTarget[0] = textX <= point.coords[0] ? textX : point.coords[0];
        point.clickTarget[1] =
          textX >= point.coords[0]
            ? textX + dimensions.points.label.width * scale * dimensions.pixelRatio
            : point.clickTarget[1];
      }
      ...
      var hThickness =
          (12 - 8 * (point.hover ? (25 - dimensions.points.progress) / 25 : 1)) *
          scale *
      ...
      context.lineTo(
        hX + hSide + hSide * dimensions.points.xAngle,
        hY + hSide * dimensions.points.yAngle,
      );
      ...
      context.drawImage(
        point,
        textX,
        y + side * dimensions.points.yAngle - 12 * scale * dimensions.pixelRatio,
        dimensions.points.label.width * scale * dimensions.pixelRatio,
        dimensions.points.label.height * scale * dimensions.pixelRatio,
      );
    }
    context.globalAlpha = 1;
    ...
    if (!point.targetLayer) {
      ...
      context.moveTo(x, y);
      context.lineTo(
        x + eHeight * 2 * dimensions.points.xAngle,
  ...
  function positionLayer(layer, offsetZ, tilt) {
    ...
    switch (layer.type) {
      case 'image':
        ...
        layer.height =
          dimensions.height * scale * layer.baseScale + (layer.baseCoords[3] ? tilt * 0.2 : 0);
        break;
      case 'point':
        layer.width = dimensions.points.width * scale * dimensions.pixelRatio;
        layer.height =
          dimensions.points.width * scale * dimensions.pixelRatio * dimensions.points.yAngle;
        break;
      case 'pattern':
      case 'gradient':
    ...
    layer.coords = [
      ...
      dimensions.halfHeight -
        layer.height / 2 +
        layer.baseCoords[1] * dimensions.height * scale +
        tilt * scale,
    ];
    if (layer.type === 'point') {
      layer.coords[2] =
        dimensions.halfWidth - layer.width / 2 + layer.baseCoords[3] * dimensions.width * scale;
      layer.coords[3] =
        dimensions.halfHeight -
        layer.height / 2 +
        layer.baseCoords[4] * dimensions.height * scale +
        tilt * scale;
    }
    if (layer.baseCoords[4] && layer.type !== 'point') {
      layer.coords[1] =
        dimensions.halfHeight -
        layer.height / 2 -
        (1 - tilt / getTilt(-dimensions.totalHeight)) * dimensions.height;
    }
  ...
  function getPointTarget(event) {
    ...
    for (var i = 0; i < layers.length; i++) {
      if (layers[i].type === 'point') {
        if (
          layers[i].clickTarget &&
          x >= layers[i].clickTarget[0] &&
          x <= layers[i].clickTarget[1] &&
          y >= layers[i].clickTarget[2] &&
          y <= layers[i].clickTarget[3]
        ) {
          layers[i].hover = true;
          target = layers[i];
        } else {
